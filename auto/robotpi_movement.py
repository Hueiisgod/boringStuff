...
class Movement:
    ...
    def move_forward(self, speed=50, times=500):
        if self.isOpen:
    ...
    def move_left(self, speed=20, times=300):
        if self.isOpen:
    ...
    def move_right(self, speed=20, times=300):
        if self.isOpen:
    ...
    def left_ward(self):
        if self.isOpen:
            command = self.cmd.Command(0, 7, 70, 500)
            self.action.write_serial(command)
            return True
        return False
